...
(function() {
  var ARG_MASK, ASSIGN_MASK, CALL_MASK, FILE_IDX_INC, HelperProcess, INDEX_MASK, Lexer, PARAM_MASK, VAR_MASK, crypto, dbg, fs, gitParser, log, net, path, util, utils;
  ...
  HelperProcess = (function() {
    function HelperProcess() {
      ...
      server.listen(utils.pipePath, (function(_this) {
        return function() {
    ...
    HelperProcess.prototype.checkFile = function(filePath) {
      var e, isCS, j, len, rawTokens, source, tokenArr, tokens;
      if ((isCS = path.extname(filePath).toLowerCase() === '.coffee')) {
        ...
        try {
          ...
          if (e.toString().indexOf('SyntaxError') === -1) {
            ...
            this.parseError(filePath, e);
          }
          return;
        }
        for (j = 0, len = rawTokens.length; j < len; j++) {
          tokenArr = rawTokens[j];
          tokens.push(this.parseToken(tokenArr));
        }
        if (!this.debug) {
    ...
    HelperProcess.prototype.addWordFileToTrie = function(word, file) {
      var fileIndex, fileIndexes, idx, j, len, newFileIndexes, newLen, node, oldLen;
      this.indexesAdded++;
      node = this.getAddWordNodeFromTrie(word);
    ...
    HelperProcess.prototype.parseError = function(filePath, e) {
      var msg;
      log('parse err', filePath, util.inspect(e, {
        depth: null
      }));
      msg = 'Coffeescript Error\nfile skipped:' + filePath + '\n' + e.toString() + ' at row:col ' + e.location.first_line + ':' + e.location.first_column + ' (to ' + e.location.last_line + ':' + e.location.last_column + ')';
      return this.broadcast('syntaxError', {
        msg: msg
      });
    };

    HelperProcess.prototype.parseToken = function(tokenArr) {
      var prop, ref, token, tokenProp, val;
      for (tokenProp in tokenArr) {
        val = tokenArr[tokenProp];
        switch (tokenProp) {
          case '0':
            token = {
              type: val
          ...
          case '1':
            ...
            break;
          case '2':
            token.row1 = val.first_line, token.row2 = val.last_line, token.col1 = val.first_column, token.col2 = val.last_column;
            break;
          default:
            token[tokenProp] = val;
        }
      ...
      if (token.origin) {
        ...
        for (prop in ref) {
          ...
          switch (prop) {
            case '0':
              token.originText = val;
              break;
            case '1':
              token.originType = val;
              break;
            case '2':
              if (val) {
                token.originRow1 = val.first_line, token.originRow2 = val.last_line, token.originCol1 = val.first_column, token.originCol2 = val.last_column;
              }
              token.originCol2 += 1;
          }
        }
