...
(function() {
  var ARG_MASK, ASSIGN_MASK, CALL_MASK, FILE_IDX_INC, HelperProcess, INDEX_MASK, PARAM_MASK, VAR_MASK, crypto, dbg, fs, gitParser, log, net, path, util, utils;
  ...
  HelperProcess = (function() {
    ...
    HelperProcess.prototype.checkFile = function(filePath) {
      var allWords, e, file, fileIndex, fileMd5, fileTime, idx, j, k, l, len, len1, len2, oldFile, ref, results, stats, text, word, wordsAssign, wordsAssignList, wordsNone, wordsNoneList;
      this.filesChecked++;
      try {
      ...
      wordsAssign = {};
      wordsNone = {};
      text.replace(/(\S+)(\s|$)/g, function(match, word, __, ofs) {
        var after, before;
        if (!(word in wordsAssign)) {
          before = text.slice(0, ofs);
          after = text.slice(ofs + word.length);
